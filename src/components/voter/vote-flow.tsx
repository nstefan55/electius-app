            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-xs text-neutral-600">
                  {t("confirmed.hashLabel")}
                </p>
                <p className="mt-1 break-all font-mono text-sm text-neutral-800">
                  {phase.voteHash}
                </p>
              </div>
              {/* 32px vizualno (§7.17), 44px doticaj (§10) preko ::after. */}
              <button
                type="button"
                onClick={() => copyHash(phase.voteHash)}
                aria-label={t("confirmed.copyHash")}
                className={`relative flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-md transition-colors after:absolute after:-inset-1.5 after:content-[''] ${copied ? "bg-success-50 text-success-700" : "text-neutral-600 hover:bg-neutral-200"}`}
              >
                {copied ? (
      ...
      <VoterCard>
        ...
        {step === 2 && (
          <>
            ...
            <div className="flex flex-col gap-2.5 rounded-lg border border-neutral-200 bg-neutral-50 p-4 text-sm">
